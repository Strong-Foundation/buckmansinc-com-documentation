...
func getAllPDFContentFromHTML(content string) []string {
	// Regular expression to match the download URLs
	completeRegex := regexp.MustCompile(`/DownloadManager/Download\.aspx\?DownloadID=\d+`)
	// Find all matches
	matches := completeRegex.FindAllString(content, -1)
	// return all the urls found.
	return matches
}
...
func main() {
	...
	givenURL := []string{
		"https://www.buckmansinc.com/DownloadManager/index.aspx?DownloadCategoryID=36",
		"https://www.buckmansinc.com/DownloadManager/index.aspx?DownloadCategoryID=38",
		"https://www.buckmansinc.com/DownloadManager/index.aspx?DownloadCategoryID=34",
		"https://www.buckmansinc.com/DownloadManager/index.aspx?DownloadCategoryID=35",
	}
	...
	extractedURL = getAllPDFContentFromHTML(fileContent)
	// Remove duplicates
	...
	for _, url := range extractedURL { // Print each extracted URL
		var finalURL string
		if !strings.HasPrefix(url, "https://www.buckmansinc.com") {
			finalURL = "https://www.buckmansinc.com" + url
		} else {
			finalURL = url
		}
		// Download the file and if its sucessful than add 1 to the counter.
		sucessCode, err := downloadPDF(finalURL, outputDir)
		if sucessCode {
